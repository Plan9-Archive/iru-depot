...
struct URL
{
	...
	char	*postbody;
	char	*cred;
	UserPasswd *up; /* internal use only */
	long	mtime;
};
...
crackurl(URL *u, char *s)
{
	...
	if(p){
		*p = 0;
		p = strchr(s, ':');
		if(p == nil){
			werrstr("bad url, null user/pass");
			return -1;
		}
		*p = 0;
		
		up = malloc(sizeof(UserPasswd));
		if(up == nil)
			sysfatal("malloc: %r");
...
void
hhetag(char *p, URL *u, URL *px, Range *r)
{
	USED(r);
	USED(px);
...
void
hhmtime(char *p, URL *u, URL *px, Range *r)
{
...
void
hhclen(char *p, URL *u, URL *px, Range *r)
{
	USED(u);
	USED(px);
...
void
hhcrange(char *p, URL *u, URL *px, Range *r)
{
	char *x;
	vlong l;
...
void
hhuri(char *p, URL *u, URL *px, Range *r)
{
	USED(r);
	USED(px);
...
void
hhlocation(char *p, URL *u, URL *px, Range *r)
{
	USED(r);
	USED(px);
...
void
hhauth(char *p, URL *u, URL *px, Range *r)
{
	char *f[4];
	UserPasswd *up;
	char *s, cred[64];
	...
	if(px && px->up){
		...
		if ((up = auth_getuserpasswd(auth_getkey, "user? !password? proto=pass service=http dom=%q relm=%q role=client",
			px->host, f[1])) == nil)
			sysfatal("cannot authenticate %r");
		u->up = up;
	}
